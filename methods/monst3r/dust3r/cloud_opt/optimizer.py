...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def optimise_pose_graph(self,
                            loop_edges: List[Tuple[int, int, torch.Tensor, float]] | None = None,
                            odom_sigma_t: float = 0.02,
                            odom_sigma_r_deg: float = 1.0,
                            iters: int = 100,
                            fix_first: bool = True) -> None:
        """
        1. Builds a PoseGraph with **all current camera parameters** as the
           initial guess.
        2. Adds *odometry* edges between consecutive frames.
        3. Adds *loop* edges supplied by `loop_edges` (if any).
        4. Runs a sparse LM solver (pyg2o when available).
        5. Writes the refined poses back into `self.im_poses`
           so the subsequent dense optimiser starts from a closed loop.

        If pyg2o is not present the function prints a warning and returns
        immediately – so nothing changes and your old pipeline keeps running.
        """
        # -------------------------------------------------- #
        # Build the graph
        # -------------------------------------------------- #
        N = self.n_imgs
        pg = PoseGraph(N, device=self.device)
        T_init = self.get_im_poses().detach()           # (N,4,4)
        pg.T[:] = T_init                                # initial estimate

        # ❶ Odometry: consecutive key-frames
        kf_ids = list(range(N))
        pg.add_odometry(kf_ids, T_init,
                        sigma_t=odom_sigma_t,
                        sigma_r=math.radians(odom_sigma_r_deg))

        # ❷ Optional loop-closure edges
        if loop_edges:
            for i, j, T_ij, w in loop_edges:
                info = torch.eye(6, device=self.device) * w
                pg.add_edge(i, j, T_ij.to(self.device), info)

        # ❸ Optimise (silently does nothing if pyg2o missing)
        pg.optimise(iters=iters, fix_first=fix_first)

        # -------------------------------------------------- #
        # Copy the refined Sim(3) back into our parameter list
        # -------------------------------------------------- #
        with torch.no_grad():
            for idx in range(N):
                self._set_pose(self.im_poses, idx, pg.T[idx], force=True)

        if self.verbose:
            print("[PointCloudOptimizer] pose-graph refinement complete.")
    def add_loop_edges(self,
                       loop_edges: List[Tuple[int, int, torch.Tensor, float]]) -> None:
        """
        Extends the optimiser graph with externally computed Sim(3) loop edges.

        Each element = (i, j, T_ij 4x4 tensor, weight float).

        * Keeps all original functionality when called with an *empty* list.
        * Re-computes internal cached tensors (edges, _ei/_ej, etc.) only if
          new edges have been supplied.

        Call *after* construction, e.g.
            opt = PointCloudOptimizer(...)
            opt.add_loop_edges(loop_edges)
        """
        if not loop_edges:
            return  # --- no-op, preserves original behaviour ---

        # 1) append to self.edges
        N0 = len(self.edges)
        for i, j, T, w in loop_edges:
            self.edges.append((i, j))
            key = edge_str(i, j)
            # fake per-edge tensors with zeros (not used in loop constraints);
            # they must exist so that forward() indexing stays valid.
            dummy = torch.zeros_like(next(iter(self.pred_i.values())))
            self.pred_i[key] = dummy
            self.pred_j[key] = dummy
            self.conf_i[key] = torch.zeros_like(next(iter(self.conf_i.values())))
            self.conf_j[key] = torch.zeros_like(next(iter(self.conf_j.values())))

            # expand pw_* learnable params
            new_pw = torch.zeros_like(self.pw_poses[0:1])
            new_adp = torch.zeros_like(self.pw_adaptors[0:1])
            self.pw_poses = nn.Parameter(torch.cat([self.pw_poses, new_pw], 0))
            self.pw_adaptors = nn.Parameter(torch.cat([self.pw_adaptors, new_adp], 0))

        # 2) refresh cached tensors used in forward()
        self.register_buffer('_ei', torch.tensor([i for i, j in self.edges]))
        self.register_buffer('_ej', torch.tensor([j for i, j in self.edges]))

        # any bookkeeping that assumed self.n_edges
        self.n_imgs = max(max(i, j) for i, j in self.edges) + 1
        self.total_area_i = sum([self.im_areas[i] for i, j in self.edges])
        self.total_area_j = sum([self.im_areas[j] for i, j in self.edges])

        if self.verbose:
            print(f'[optimizer] added {len(loop_edges)} loop edges '
                  f'(graph size {N0} → {len(self.edges)})')
    ...
    def forward_non_batchify(self, epoch=9999):
        print("👾👾👾 Entered Forward() Pass of the PointCloudOptimizer class 👾👾👾")
