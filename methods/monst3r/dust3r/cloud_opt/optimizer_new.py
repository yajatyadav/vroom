...
class PointCloudOptimizerMultiChunk(PointCloudOptimizer):
    def __init__(self, scene_list, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # easy ones first

    def _stitch_depthmaps(self) -> None:
        """Fill ``self.im_depthmaps`` with the optimized depth maps
        coming from each chunk‐level optimizer."""
        global_offset = 0
        for opt in self.chunk_opts:
            depth_list = list(opt.im_depthmaps)
            
            for local_idx, depth_param in enumerate(depth_list):
                global_idx = global_offset + local_idx
                self.im_depthmaps[global_idx].data.copy_(depth_param.data)

            global_offset += len(depth_list)

        assert global_offset == self.n_imgs, "chunk lengths don’t add up!"
