...
class FactorGraph:
    ...
    def __filter_repeated_edges(self, ii, jj):
        ...
        for i, j in zip(ii, jj):
            if (i.item(), j.item()) not in eset:
                ii_new.append(i)
                jj_new.append(j)
        return torch.as_tensor(ii_new, device=ii.device), torch.as_tensor(jj_new, device=jj.device)
    ...
    def add_neighborhood_factors(self, t0, t1, r=3):
        ...
        import ipdb; ipdb.set_trace()
        self.add_factors(ii, jj) ## TODO(YY): CHANGED
