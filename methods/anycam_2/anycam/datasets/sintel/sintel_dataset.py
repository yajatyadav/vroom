...
class SintelDataset(Dataset):
    ...
    def _get_sequences(data_path: str):
        sequences = {}
        for seq in os.listdir(os.path.join(data_path, "final")):
            if os.path.isdir(os.path.join(data_path, "final", seq)):
                seq_len = len(os.listdir(os.path.join(data_path, "final", seq)))
        ...
        for id in ids:
            img = (
                cv2.cvtColor(
                    cv2.imread(
                        os.path.join(
                            self.data_path, "final", seq, f"frame_{id+1:04d}.png"
                        )
                    ),
                    cv2.COLOR_BGR2RGB,
                ).astype(np.float32)
    ...
    def load_depth(self, seq, id, target_size=None, crop=None):
        ...
        depth_path = os.path.join(self.data_path, "depth", seq, f"frame_{id+1:04d}.dpt")
...
class SintelGTDataset(SintelDataset):
    NAME = "Sintel GT"

    def __len__(self):
        return 38

    def _read_flo(self, file_name):
    ...
    #     for (fwd_id, fwd_is_fwd), (bwd_id, bwd_is_fwd) in zip(*flow_ids):
    #         flow_fwd = self._read_flo(os.path.join(self.data_path, "flow", seq, f"frame_{fwd_id+1:04d}.flo"))
    ...
    def __getitem__(self, index):
        ...
        if self.return_depth:
            depth = np.stack([self.load_depth(sequence, id, target_size, crop) for id in ids]) #* 0.1
            data["depths"] = depth
