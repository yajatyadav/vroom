...
class DavisDataset(Dataset):
    ...
    def _get_sequences(data_path: str):
        sequences = {}
        for seq in os.listdir(os.path.join(data_path, "JPEGImages", "480p")):
            if os.path.isdir(os.path.join(data_path, "JPEGImages", "480p", seq)):
                img_list = list(sorted(os.listdir(os.path.join(data_path, "JPEGImages", "480p", seq))))
        ...
        for id in ids:
            img = (
                cv2.cvtColor(
                    cv2.imread(
                        os.path.join(
                            self.data_path, "JPEGImages", "480p", seq, f"{id:05d}.jpg"
                        )
                    ),
                    cv2.COLOR_BGR2RGB,
                ).astype(np.float32)
