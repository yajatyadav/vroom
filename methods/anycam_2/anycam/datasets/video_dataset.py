...
class VideoDataset(Dataset):
    ...
    def load_flows(self, seq, ids):
        ...
        for (fwd_id, fwd_is_fwd), (bwd_id, bwd_is_fwd) in zip(*flow_ids):
            flow_fwd = load_flow(self.make_flow_path(seq, fwd_id, fwd_is_fwd))
            flow_bwd = load_flow(self.make_flow_path(seq, bwd_id, bwd_is_fwd))
    ...
    def get_img_paths(self, index):
        ...
        img_paths = [
            self.make_img_path(sequence, id)
            for id in ids
        ]
