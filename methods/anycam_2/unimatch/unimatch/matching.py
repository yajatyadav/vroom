...
def global_correlation_softmax(feature0, feature1,
    ...
    correspondence = torch.matmul(prob.to(init_grid.dtype), grid).view(b, h, w, 2).permute(0, 3, 1, 2)  # [B, 2, H, W]
...
def local_correlation_softmax(feature0, feature1, local_radius,
    ...
    window_feature = F.grid_sample(feature1.contiguous(), sample_coords_norm.to(feature1.dtype).contiguous(),
    ...
    corr[~valid] = torch.finfo(torch.float16).min

    prob = F.softmax(corr, -1)  # [B, H*W, (2R+1)^2]

    correspondence = torch.matmul(prob.unsqueeze(-2).to(sample_coords_softmax.dtype), sample_coords_softmax).squeeze(-2).view(
        b, h, w, 2).permute(0, 3, 1, 2)  # [B, 2, H, W]

    flow = correspondence.to(coords_init.dtype) - coords_init
    match_prob = prob
...
def local_correlation_with_flow(feature0, feature1,
    ...
    # normalize coordinates to [-1, 1]
    sample_coords_norm = normalize_coords(sample_coords, h, w)  # [-1, 1]
    window_feature = F.grid_sample(feature1, sample_coords_norm.to(feature1.dtype),
                                   padding_mode=padding_mode, align_corners=True
                                   ).permute(0, 2, 1, 3)  # [B, H*W, C, (2R+1)^2]
    feature0_view = feature0.permute(0, 2, 3, 1).view(b, h * w, 1, c)  # [B, H*W, 1, C]
