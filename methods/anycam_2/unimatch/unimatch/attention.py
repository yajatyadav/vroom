...
def single_head_split_window_attention(q, k, v,
    ...
    out = F.scaled_dot_product_attention(q.view(b_new, -1, c), k.view(b_new, -1, c), v.view(b_new, -1, c), attn_mask.repeat(b, 1, 1))
...
class SelfAttnPropagation(nn.Module):
    ...
    def forward_local_window_attn(self, feature0, flow,
        ...
        out = F.scaled_dot_product_attention(feature0_reshape, feature0_window.permute(0, 2, 1), flow_window)

        
        out = out.view(b, h, w, value_channel).permute(0, 3, 1, 2).contiguous()  # [B, 2, H, W]
